...
**Status sinkronisasi:** 29 Juli 2026  
**Pasangan kode terbaru:** `index(54).html`  
**SHA-256 pasangan kode:** `ddf18dc30d6d9de3de31e7c9947e1bc7d231c6bc1a9578311845b63079844bfd`
...
Snapshot ini dibuat dari `index(54).html` yang dipasangkan dengan prompt ini.
...
- Navigasi desktop menggunakan sidebar bertingkat.
- Navigasi mobile menggunakan bottom navigation dan harus tetap sinkron dengan sidebar.
...
| Grup | Panel | Sumber/ketergantungan utama |
|---|---|---|
| Daily | Daily Schedule | `schedule_events`, public + private |
| Daily | Time Plan Tracker | Statistik dan Project/Milestone memakai `time_plan_tracker`; `tracker_items` masih dipakai oleh Result dan fungsi lama terkait |
| Daily | Kanban Board | Visualisasi dan perubahan status dari `time_plan_tracker` |
| Daily | Gantt Chart | Timeline dari `time_plan_tracker` |
| Daily | Decision Matrix | Gabungan To Do belum selesai dari `todos` dan item aktif `time_plan_tracker` |
| Daily | Recurring Task | `recurring_templates`; dapat membuat item ke `schedule_events` atau `time_plan_tracker` |
| Daily | Logbook | `logbook_entries`, lampiran Supabase Storage, dan tautan ke `time_plan_tracker` |
| Tasks | Notes | `notes`; selalu private dan hanya milik user aktif |
| Tasks | To Do | `todos`; hanya milik user aktif |
| Tasks | Notepad | `notepad_items`; hanya milik user aktif |
...
| Quick Links | All Links | `app_links`; data bersama tim |
| Quick Links | Password Manager | `vault_keys` dan `password_vault_v2`; private per user dan terenkripsi end-to-end |
...
- Masih mungkin ada teks Indonesia lama di `index(54).html`; itu adalah technical debt, bukan alasan mengubah isi data user.
...
### 15.8 Today Focus dan Quick Capture

- `Today Focus` menjadi panel awal ketika WorkBoard dibuka pada desktop dan mobile.
- Panel ini memakai tabel `todos` yang sudah ada; tidak memerlukan tabel atau kolom database baru.
- Data tetap private dan difilter dengan `applyOwner()` untuk user aktif.
- `Quick Capture` menyimpan teks singkat langsung sebagai To Do private dengan:
  - deadline hari ini,
  - priority `med`,
  - status belum selesai.
- Panel menampilkan maksimal tiga tugas utama berdasarkan urutan: overdue, jatuh tempo hari ini, indikasi follow-up, dan priority.
- Bagian `Follow Up Today` mendeteksi kata kerja follow-up sederhana dalam Bahasa Indonesia atau English dari teks To Do.
- Bagian `Waiting for Reply` mendeteksi item menunggu dari teks To Do.
- Setiap kartu menyediakan tindakan `Done`, `Tomorrow`, dan `Open`.
- Badge Today Focus menunjukkan jumlah gabungan tugas overdue dan jatuh tempo hari ini.
- Panel To Do lama, dashboard bulanan, autosave draft, serta struktur tabel `todos` tetap dipertahankan.
- Today Focus ikut diperbarui ketika perubahan realtime pada tabel `todos` diterima saat panel tersebut aktif.
...
- [ ] Notes, To Do, Notepad, dan Password Manager tetap owner-only.
